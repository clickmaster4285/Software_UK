...
export function CaseStudyCard({
  ...
  const content = (
    <Card className="py-0 group h-full overflow-hidden border border-slate-200/80 bg-white rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-400">
      {/* Header Strip */}
      ...
      <div className="relative h-44 overflow-hidden bg-gradient-to-br from-slate-50 to-slate-100 border-b border-slate-100">
        {thumbnail ? (
          ...
          <div className="w-full h-full flex items-center justify-center relative">
            {/* Decorative circles */}
            <div className="absolute top-4 right-4 w-24 h-24 bg-accent/5 rounded-full" />
            <div className="absolute bottom-4 left-4 w-16 h-16 bg-primary/5 rounded-full" />

            <div className="relative w-16 h-16 bg-white rounded-2xl shadow-md flex items-center justify-center">
              <span className="text-3xl">{emoji}</span>
            </div>

            {/* Category badge */}
            <div className="absolute top-4 left-4">
              <span className="inline-flex items-center gap-1.5 bg-white/90 backdrop-blur-sm text-slate-700 px-3 py-1 rounded-full text-[11px] font-semibold shadow-sm border border-slate-200/50">
      ...
      <CardContent className="p-6 flex flex-col gap-4 flex-1">
        {/* Title */}
        <h3 className="text-lg font-bold text-slate-900 leading-snug line-clamp-2 group-hover:text-primary transition-colors duration-300">
          {title}
        </h3>
        ...
        {technologies && technologies.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {technologies.map((tech) => (
              <span
                key={tech}
                className="inline-flex items-center gap-1 px-2 py-0.5 bg-slate-100 text-slate-500 rounded-md text-[10px] font-medium"
              >
          ...
          {challenge && (
            <div className="rounded-lg bg-red-50 border border-red-100 p-2.5">
              <div className="flex items-center gap-1.5 text-red-600 mb-1">
                ...
                <span className="text-[9px] font-bold uppercase tracking-wider">Challenge</span>
              </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-100 mt-auto">
          <span className="text-xs font-semibold text-slate-400 group-hover:text-accent transition-colors duration-300 uppercase tracking-wider">
            Read Full Story
          </span>
