...
export function ComparisonDetailClient({ comparison, relatedComparisons }) {
  ...
  const topicDisplay = topic.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="relative max-w-[96vw] lg:max-w-[90vw] mx-auto px-6">
          ...
          <div className="max-w-7xl">
            <span className="section-label mb-5 bg-white/10 border-white/15 text-white/80">
              <GitCompare className="w-3 h-3 mr-1.5 inline -mt-0.5" />
              {topicDisplay}
            </span>
      ...
      {comparisonTables.map((table, tableIdx) => (
        <section key={tableIdx} className="border-b border-border bg-white">
              ...
              <table className="w-full text-sm md:text-base">
                  ...
                  <thead>
                    <tr className="bg-primary-mid/5">
                      {table.headers.map((header, hIdx) => (
                        <th
                          key={hIdx}
                          className={`px-4 md:px-6 py-3 md:py-4 text-left font-heading font-bold text-text-primary text-xs md:text-sm uppercase tracking-wider ${hIdx > 0 ? 'border-l border-border' : ''}`}
                        >
                          {header}
                        </th>
                      ))}
                ...
                <tbody>
                  {table.rows.map((row, rIdx) => (
                    <tr
                      key={rIdx}
                      className={`${rIdx % 2 === 1 ? 'bg-surface/50' : 'bg-white'} hover:bg-accent/5 transition-colors`}
                    >
                      {row.map((cell, cIdx) => (
                        <td
                          key={cIdx}
                          className={`px-4 md:px-6 py-3 md:py-4 text-text-body ${cIdx === 0 ? 'font-semibold text-text-primary whitespace-nowrap' : ''} ${cIdx > 0 ? 'border-l border-border' : ''}`}
                        >
                          {cell}
                        </td>
                      ))}
      ...
      {bodySections.map((section, sIdx) => (
        <section key={sIdx} className={`border-b border-border ${sIdx % 2 === 0 ? 'bg-surface' : 'bg-white'}`}>
          <div className="max-w-[96vw] lg:max-w-[90vw] mx-auto px-6 py-12 md:py-16">
            {section.title && (
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <div className="space-y-3">
                {faqs.map((faq, idx) => (
                  <div
                    key={idx}
                    className="rounded-xl border border-white/10 bg-white/5 overflow-hidden transition-all duration-250"
                  >
                    <button
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`w-5 h-5 text-accent shrink-0 transition-transform duration-250 ${openFaq === idx ? 'rotate-180' : ''}`}
                      />
                    </button>
                    {openFaq === idx && (
          ...
          <div className="grid md:grid-cols-2 gap-8 md:gap-12">
            {author && (
              <div>
                ...
                {writtenBy && (
                  <p className="text-text-muted text-sm mt-1">{writtenBy}</p>
                )}
                {reviewedBy && (
                ...
                <ul className="space-y-2">
                  {relatedPages.map((page, idx) => (
                    <li key={idx}>
                      <Link
                        href={`/${page.slug}`}
                        className="flex items-center gap-2 text-text-body hover:text-accent transition-colors text-sm"
                      >
        ...
        <section className="border-b border-border bg-surface">
          <div className="max-w-[96vw] lg:max-w-[90vw] mx-auto px-6 py-12 md:py-16">
            <div className="max-w-4xl mx-auto text-center">
              <p className="text-text-body text-lg leading-relaxed">{cta}</p>
            </div>
          </div>
        </section>
      )}
        ...
        <section className="py-16 px-6 bg-surface">
          <div className="max-w-[96vw] lg:max-w-[90vw] mx-auto">
            <div className="flex items-center justify-between mb-10">
              ...
              <Link href="/comparison" className="hidden md:flex items-center gap-2 text-sm font-semibold text-accent hover:gap-3 transition-all">
                View All <ArrowRight className="w-4 h-4" />
              </Link>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {relatedComparisons.map((rc, idx) => (
                <Link
                  key={rc.id}
                  href={`/comparison/${rc.slug}`}
                ...
                >
                  <h3 className="font-heading text-sm font-bold text-text-primary group-hover:text-accent transition-colors line-clamp-2 mb-2">
                    {rc.title}
                  </h3>
                  <p className="text-xs text-text-muted line-clamp-2">
                    {rc.metaDesc?.substring(0, 100)}
                  </p>
            ...
            <div className="mt-8 text-center md:hidden">
              <Link href="/comparison" className="btn-primary inline-flex items-center gap-2">
        ...
        <div className="max-w-[96vw] lg:max-w-[90vw] mx-auto">
          <div className="relative overflow-hidden rounded-3xl bg-linear-to-br from-primary via-primary-mid to-primary-light px-8 py-16 md:px-16 md:py-20 text-center">
            ...
            <div className="relative">
              <span className="section-label mb-6 bg-white/10 border-white/15 text-white/80">Need Help Deciding?</span>
              <h2 className="font-heading text-3xl md:text-4xl font-bold text-white mb-4">Get Unbiased Technology Advice</h2>
              <p className="text-white/60 max-w-xl mx-auto mb-10 text-lg">Our architects are certified across all major platforms. We recommend what's actually right for your business — no commission, no bias.</p>
