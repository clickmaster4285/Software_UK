...
/**
 * Strip MD scaffold CTAs / schema labels that leak into body & FAQ copy.
 * Returns { text, primaryCta, secondaryCta }.
 */
export function stripCtaArtifacts(text) {
  if (!text || typeof text !== "string") {
    return { text: "", primaryCta: null, secondaryCta: null };
  }
  const src = cleanMojibake(text);
  // MD forms: **Primary CTA:** Label  |  **Primary CTA**: Label  |  Primary CTA: Label
  ...
  const scrub = (raw) =>
    String(raw || "")
      ...
      .trim()
      .replace(/\s+/g, " ");
  const primaryCta = primaryMatch ? scrub(primaryMatch[1]) : null;
  const secondaryCta = secondaryMatch ? scrub(secondaryMatch[1]) : null;

  const cleaned = src
    .replace(/\*{0,2}\s*Primary\s+CTA\s*:?\s*\*{0,2}\s*:?\s*[^\n]*/gi, " ")
    .replace(/\*{0,2}\s*Secondary\s+CTA\s*:?\s*\*{0,2}\s*:?\s*[^\n]*/gi, " ")
  ...
  const trimLabel = (v) =>
    v && v.length > 2 && v.length < 90 ? v.replace(/[.]+$/, "").trim() : null;

  return {
    text: cleaned,
    primaryCta: trimLabel(primaryCta),
    secondaryCta: trimLabel(secondaryCta),
  };
}
...
export function linkifyMarkdown(text) {
  if (!text) return "";
  const { text: cleaned } = stripCtaArtifacts(String(text));
